...
1. Read the [validation result] to extract the items.
2. Count the exact number of bullet points in the [validation result]. Your output array MUST contain exactly that many items. Do not invent products or split items.
3. Compute the math textually: subtotal is (quantity * unit_price). Total is the sum of all subtotals.
4. Get the status stated in [validation result], and fill in the status.
5. Write a helpful "remark" sentence if an item's status is AMBIGUOUS or UNLISTED in [validation result]. Otherwise, use null.
